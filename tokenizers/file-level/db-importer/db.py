...
class DB:
    ...
    def insert_file(self, file_id, proj_id, file_path, file_url, file_hash, flush=False, autoID=False):
        if not flush:
            if file_url is None:
                file_url = 'NONE'
            self.files.append(files_list % (
            file_id, proj_id, self.sanitize_string(file_path), self.sanitize_string(file_url), file_hash))
    ...
    def insert_block(self, project_id, file_id, block_id, block_hash, block_start_line, block_end_line, flush=False):
        if not flush:
            self.blocks.append(
                blocks_list % (project_id, file_id, block_id, block_hash, block_start_line, block_end_line))
    ...
    def sanitize_string(self, string_input):
        ...
        return "".join(string_res[:DB_MAX_STRING_SIZE])
